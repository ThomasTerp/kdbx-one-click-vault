...
export default function VaultEntryFieldCopyButton({ entryUUID, entryFieldData }: { entryUUID: string; entryFieldData: VaultEntryFieldData }) {
	...
	return (
		<div
			className={cn(
				"relative flex flex-col px-2 py-1 text-sm text-card-foreground cursor-pointer rounded-md overflow-hidden hover:bg-accent",
				isCopied && "bg-accent"
			)}
			onClick={() => {
				void onClick();
		...
		>
			...
			{isProtected ? (
				...
				<div className="max-w-64 truncate">{field}</div>
			)}
			{isCopied && (
